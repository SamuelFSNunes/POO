...
public class ScreenProtector extends javax.swing.JFrame implements Runnable{
    ...
    public void run() {
        ...
        /*rand.nextInt(100);
        Rectangle [] r = new Rectangle[quant];
        Square [] s = new Square [quant];
        Ellipse [] e = new Ellipse[quant];
        Circle [] c = new Circle[quant];

        for (int i = 0 ; i<quant ; i++){
            r[i] = new Rectangle(colorGenerate(),
        ...
        Image logo = new Image("/images/Carro4visaolateral1.gif", 300,200,"/sounds/impact.wav",false);
        logo.scale(5);




        // Como em um desenho animado, as animações são criadas a partir da sobreposição de frames
        while (true) {
            ...
            // ---------------Desenha a imagem, na escala desejada
            logo.draw(g);
            if (right)
                logo.moveX(3);
            if (left)
           ...
           /* for (Rectangle rect : r){
                rect.draw(g);
                rect.move();
                rect.changeDir(0,getWidth(),0,getHeight());
            }
            for (Ellipse rect : e){
                rect.draw(g);
                rect.move();
    ...
    //   Este método gera uma cor aleatória
...
} // Fin da Classe Screen Protector
